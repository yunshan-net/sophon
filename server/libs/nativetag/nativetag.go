...
func UpdateNativeTag(op NativeTagOP, orgId uint16, table NativeTagTable, nativeTag *NativeTag) {
	...
	NativeTags[orgId][table] = oldNativeTag
	log.Infof("after %s orgid %d, table %s, native tag: %+v", op, orgId, table, oldNativeTag)
}
