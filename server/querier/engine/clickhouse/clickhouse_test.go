...
var (
	parseSQL = []struct {
		...
		output: []string{"SELECT AVG(byte_tx) AS `aavg_byte_tx` FROM flow_log.`l4_flow_log` PREWHERE `time` >= 60 AND `time` <= 180 HAVING minus(MAX(byte_tx), MIN(byte_tx)) >= 0 LIMIT 1"},
	}, {
		input:  "select Avg(byte_tx) as avg_byte_tx from l4_flow_log where `time`>=60 and `time`<=180 having Spread(byte_tx)>=0 limit 1",
		output: []string{"SELECT sum(byte_tx)/(121/1) AS `avg_byte_tx` FROM flow_log.`l4_flow_log` PREWHERE `time` >= 60 AND `time` <= 180 HAVING minus(MAX(byte_tx), MIN(byte_tx)) >= 0 LIMIT 1"},
	}, {
		...
		output: []string{"SELECT minus(MAX(byte_tx), MIN(byte_tx)) AS `spread_byte_tx` FROM flow_log.`l4_flow_log` PREWHERE `time` >= 60 AND `time` <= 180 LIMIT 1"},
	}, {
		input:  "select Rspread(byte_tx) as rspread_byte_tx from l4_flow_log where `time`>=60 and `time`<=180 limit 1",
		output: []string{"SELECT divide(MAX(byte_tx)+1e-15, MIN(byte_tx)+1e-15) AS `rspread_byte_tx` FROM flow_log.`l4_flow_log` PREWHERE `time` >= 60 AND `time` <= 180 LIMIT 1"},
	}, {
		...
		output: []string{"SELECT AVGIf(rtt, rtt > 0) AS `avg_rtt` FROM flow_log.`l4_flow_log` PREWHERE `time` >= 100 + 1 AND `time` <= 102 LIMIT 1"},
	}, {
		input:  "select AAvg(rtt) as aavg_rtt from l4_flow_log where time >= 100+1 and time <= 102 limit 1",
		output: []string{"SELECT AVGIf(rtt, rtt > 0) AS `aavg_rtt` FROM flow_log.`l4_flow_log` PREWHERE `time` >= 100 + 1 AND `time` <= 102 LIMIT 1"},
	}, {
		...
		output: []string{"SELECT service_id_0 AS `pod_service_id_0` FROM flow_log.`l7_flow_log` PREWHERE (not(service_id_0 = 3)) AND (service_id_0!=0) GROUP BY service_id_0 AS `pod_service_id_0` LIMIT 10000"},
	}, {
		input:  "select region_id_0 from l7_flow_log where pod_ingress_0 !='xx' group by region_id_0",
		output: []string{"SELECT region_id_0 FROM flow_log.`l7_flow_log` PREWHERE (not(((if(is_ipv4=1,IPv4NumToString(ip4_0),IPv6NumToString(ip6_0)),toUInt64(l3_epc_id_0)) GLOBAL IN (SELECT ip,l3_epc_id FROM flow_tag.ip_relation_map WHERE pod_ingress_name = 'xx')) OR (toUInt64(service_id_0) GLOBAL IN (SELECT pod_service_id FROM flow_tag.ip_relation_map WHERE pod_ingress_name = 'xx')))) GROUP BY `region_id_0` LIMIT 10000"},
	}, {
		input:  "select node_type(region_0) as `node_type_0`,mask(ip_0,33) as `mask_ip_0` from l7_flow_log group by `mask_ip_0`,`node_type_0`",
		output: []string{"WITH if(is_ipv4, IPv4NumToString(bitAnd(ip4_0, 4294967295)), IPv6NumToString(bitAnd(ip6_0, toFixedString(unhex('ffffffff800000000000000000000000'), 16)))) AS `mask_ip_0` SELECT 'region' AS `node_type_0`, `mask_ip_0` FROM flow_log.`l7_flow_log` GROUP BY `mask_ip_0`, `node_type_0` LIMIT 10000"},
	}, {
		input:  "select region_id_0 from l7_flow_log group by region_id_0,chost_id_1",
		output: []string{"SELECT region_id_0, if(l3_device_type_1=1,l3_device_id_1, 0) AS `chost_id_1` FROM flow_log.`l7_flow_log` PREWHERE (l3_device_id_1!=0 AND l3_device_type_1=1) GROUP BY `region_id_0`, if(l3_device_type_1=1,l3_device_id_1, 0) AS `chost_id_1` LIMIT 10000"},
	}, {
		input:  "SELECT ip_0 FROM l4_flow_log WHERE  ((is_internet_1=1) OR (is_internet_0=1)) GROUP BY ip_0 limit 1",
		output: []string{"SELECT if(is_ipv4=1, IPv4NumToString(ip4_0), IPv6NumToString(ip6_0)) AS `ip_0` FROM flow_log.`l4_flow_log` PREWHERE (((l3_epc_id_1 = -2)) OR ((l3_epc_id_0 = -2))) GROUP BY if(is_ipv4=1, IPv4NumToString(ip4_0), IPv6NumToString(ip6_0)) AS `ip_0` LIMIT 1"},
	}, {
		input:  "select Sum(byte) as `流量总量`, region_0 as `区域` from l4_flow_log where 1=1 group by `区域` order by `流量总量` desc",
		output: []string{"SELECT dictGet('flow_tag.region_map', 'name', (toUInt64(region_id_0))) AS `区域`, SUM(byte_tx+byte_rx) AS `流量总量` FROM flow_log.`l4_flow_log` PREWHERE 1 = 1 GROUP BY `区域` ORDER BY `流量总量` desc LIMIT 10000"},
	}, {
		...
		output: []string{"SELECT attribute_values[indexOf(attribute_names,'cc')] AS `attribute.abc` FROM flow_log.`l7_flow_log` PREWHERE attribute_values[indexOf(attribute_names,'cc')] = 'opensource-loki-0' AND (attribute_values[indexOf(attribute_names,'cc')] != '') GROUP BY `attribute.abc` LIMIT 10000"},
	}, {
		input:  "select `tag.cc` as `tag.abc` from cpu where `tag.abc`='opensource-loki-0' group by `tag.abc`",
		output: []string{"SELECT tag_values[indexOf(tag_names,'cc')] AS `tag.abc` FROM ext_metrics.`metrics` PREWHERE (virtual_table_name='cpu') AND tag_values[indexOf(tag_names,'cc')] = 'opensource-loki-0' AND (tag_values[indexOf(tag_names,'cc')] != '') GROUP BY `tag.abc` LIMIT 10000"},
		db:     "ext_metrics",
	}, {
		input:  "select `metrics.storageclass_annotations` AS `job_info` from prometheus_kube",
		output: []string{"SELECT if(indexOf(metrics_float_names, 'storageclass_annotations')=0,null,metrics_float_values[indexOf(metrics_float_names, 'storageclass_annotations')]) AS `job_info` FROM ext_metrics.`metrics` PREWHERE (virtual_table_name='prometheus_kube') LIMIT 10000"},
		db:     "ext_metrics",
	}, {
		input:  "select Sum(`metrics.pending`) from `deepflow_server.queue`",
		output: []string{"SELECT SUM(if(indexOf(metrics_float_names, 'pending')=0,null,metrics_float_values[indexOf(metrics_float_names, 'pending')])) AS `Sum(metrics.pending)` FROM deepflow_tenant.`deepflow_collector` PREWHERE (virtual_table_name='deepflow_server.queue') LIMIT 10000"},
		db:     "deepflow_tenant",
	}, {
		...
		output: []string{"SELECT if(indexOf(metrics_float_names, 'xxx')=0,null,metrics_float_values[indexOf(metrics_float_names, 'xxx')]) AS `xxx` FROM ext_metrics.`metrics` PREWHERE (virtual_table_name='cpu') LIMIT 10000"},
		db:     "ext_metrics",
	}, {
		input:  "select Percentile(`metrics.xxx`, 0.9) as xxx from cpu",
		output: []string{"SELECT quantile(0.9)(`_sum_if(indexOf(metrics_float_names, xxx)=0,null,metrics_float_values[indexOf(metrics_float_names, xxx)])`) AS `xxx` FROM (SELECT SUM(if(indexOf(metrics_float_names, 'xxx')=0,null,metrics_float_values[indexOf(metrics_float_names, 'xxx')])) AS `_sum_if(indexOf(metrics_float_names, xxx)=0,null,metrics_float_values[indexOf(metrics_float_names, xxx)])` FROM ext_metrics.`metrics` PREWHERE (virtual_table_name='cpu')) LIMIT 10000"},
		db:     "ext_metrics",
	}, {
		...
		output: []string{"WITH dictGetOrDefault('flow_tag.string_enum_map', 'name', ('observation_point',observation_point), observation_point) AS `Enum(tap_side)` SELECT `Enum(tap_side)` FROM flow_log.`l7_flow_log` LIMIT 0, 50"},
	}, {
		...
		output: []string{"SELECT histogramIf(10)(`_sum_byte_tx+byte_rx`,`_sum_byte_tx+byte_rx`>0) AS `histo` FROM (SELECT SUM(byte_tx+byte_rx) AS `_sum_byte_tx+byte_rx` FROM flow_log.`l4_flow_log` LIMIT 10000)"},
	}, {
		...
		output: []string{"SELECT dictGet('flow_tag.region_map', 'name', (toUInt64(region_id_0))) AS `region_0` FROM flow_log.`l7_flow_log` PREWHERE (toUInt64(region_id) GLOBAL IN (SELECT id FROM flow_tag.region_map WHERE match(name,'系统*'))) LIMIT 10000"},
	}, {
		...
		output: []string{"SELECT if(if(l3_device_type_0=1, dictGet('flow_tag.chost_cloud_tag_map', 'value', (toUInt64(l3_device_id_0),'xx')), '')!='',if(l3_device_type_0=1, dictGet('flow_tag.chost_cloud_tag_map', 'value', (toUInt64(l3_device_id_0),'xx')), ''), dictGet('flow_tag.pod_ns_cloud_tag_map', 'value', (toUInt64(pod_ns_id_0),'xx')) ) AS `cloud.tag.xx_0` FROM flow_log.`l4_flow_log` PREWHERE NOT (((toUInt64(l3_device_id_0) GLOBAL IN (SELECT id FROM flow_tag.chost_cloud_tag_map WHERE key='xx') AND l3_device_type_0=1) OR (toUInt64(pod_ns_id_0) GLOBAL IN (SELECT id FROM flow_tag.pod_ns_cloud_tag_map WHERE key='xx')))) LIMIT 1"},
	}, {
		...
		output: []string{"SELECT dictGet('flow_tag.pod_k8s_env_map', 'value', (toUInt64(pod_id_0),'statefulset.kubernetes.io/pod-name')) AS `k8s.env.statefulset.kubernetes.io/pod-name_0` FROM flow_log.`l4_flow_log` PREWHERE (toUInt64(pod_id_0) GLOBAL IN (SELECT id FROM flow_tag.pod_k8s_env_map WHERE key='statefulset.kubernetes.io/pod-name')) GROUP BY `k8s.env.statefulset.kubernetes.io/pod-name_0` LIMIT 10000"},
	}, {
		...
		output: []string{"SELECT dictGet('flow_tag.pod_k8s_env_map', 'value', (toUInt64(pod_id_0),'statefulset.kubernetes.io/pod-name')) AS `k8s.env.statefulset.kubernetes.io/pod-name_0` FROM flow_log.`l4_flow_log` PREWHERE toUInt64(pod_id_0) GLOBAL IN (SELECT id FROM flow_tag.pod_k8s_env_map WHERE value = 'opensource-loki-0' and key='statefulset.kubernetes.io/pod-name') AND (toUInt64(pod_id_0) GLOBAL IN (SELECT id FROM flow_tag.pod_k8s_env_map WHERE key='statefulset.kubernetes.io/pod-name')) GROUP BY `k8s.env.statefulset.kubernetes.io/pod-name_0` LIMIT 10000"},
	}, {
		...
		output: []string{"SELECT dictGet('flow_tag.pod_k8s_env_map', 'value', (toUInt64(pod_id_0),'statefulset.kubernetes.io/pod-name')) AS `k8s.env.abc` FROM flow_log.`l4_flow_log` PREWHERE toUInt64(pod_id_0) GLOBAL IN (SELECT id FROM flow_tag.pod_k8s_env_map WHERE value = 'opensource-loki-0' and key='statefulset.kubernetes.io/pod-name') AND (toUInt64(pod_id_0) GLOBAL IN (SELECT id FROM flow_tag.pod_k8s_env_map WHERE key='statefulset.kubernetes.io/pod-name')) GROUP BY `k8s.env.abc` LIMIT 10000"},
	}, {
		...
		output: []string{"SELECT SUM(1) AS `sum_log_count` FROM flow_log.`l7_flow_log` PREWHERE `会话长度` >= 893689408 LIMIT 10000"},
	}, {
		input:  "select session_length AS `会话长度` from l7_flow_log where `session_length`<=392037",
		output: []string{"SELECT if(request_length>0,request_length,0)+if(response_length>0,response_length,0) AS `会话长度` FROM flow_log.`l7_flow_log` PREWHERE if(request_length>0,request_length,0)+if(response_length>0,response_length,0) <= 392037 LIMIT 10000"},
	}, {
		...
		output: []string{"WITH dictGetOrDefault('flow_tag.int_enum_map', 'name', ('pod_group_type',toUInt64(dictGet('flow_tag.pod_group_map', 'pod_group_type', (toUInt64(pod_group_id_0))))), dictGet('flow_tag.pod_group_map', 'pod_group_type', (toUInt64(pod_group_id_0)))) AS `Enum(pod_group_type_0)` SELECT `Enum(pod_group_type_0)`, dictGet('flow_tag.pod_group_map', 'pod_group_type', (toUInt64(pod_group_id_0))) AS `pod_group_type_0` FROM flow_log.`l7_flow_log` PREWHERE (not(toUInt64(dictGet('flow_tag.pod_group_map', 'pod_group_type', (toUInt64(pod_group_id_0)))) GLOBAL IN (SELECT value FROM flow_tag.int_enum_map WHERE name = 'Deployment' and tag_name='pod_group_type') AND pod_group_id_0!=0)) LIMIT 10"},
	}, {
		name:   "count_1",
		...
		output: []string{"SELECT `xx`, region_0, AVG(`_sum_byte_tx`) AS `AAvg(byte_tx)`, SUM(`_count_1`) AS `c` FROM (WITH if(l3_device_type_0=1, dictGet('flow_tag.device_map', 'icon_id', (toUInt64(1),toUInt64(l3_device_id_0))), 0) AS `xx` SELECT `xx`, dictGet('flow_tag.region_map', 'name', (toUInt64(region_id_0))) AS `region_0`, SUM(byte_tx) AS `_sum_byte_tx`, COUNT(1) AS `_count_1` FROM flow_metrics.`network_map` GROUP BY `xx`, dictGet('flow_tag.region_map', 'name', (toUInt64(region_id_0))) AS `region_0`) GROUP BY `xx`, `region_0` HAVING c > 0 LIMIT 1"},
		db:     "flow_metrics",
	}, {
		name:   "topk_1",
		db:     "flow_metrics",
		...
		output: []string{"SELECT arrayStringConcat(topKIf(2)(dictGetOrDefault('flow_tag.int_enum_map', 'name', ('protocol',toUInt64(protocol)), protocol), dictGetOrDefault('flow_tag.int_enum_map', 'name', ('protocol',toUInt64(protocol)), protocol) != ''), ',') AS `TopK_2(protocol)` FROM flow_log.`l4_flow_log` LIMIT 2"},
	}, {
		name:   "select_enum",
		db:     "flow_log",
		...
		output: []string{"SELECT dictGet('flow_tag.pod_ns_map', 'name', (toUInt64(pod_ns_id))) AS `pod_ns`, any(dictGet('flow_tag.pod_map', 'name', (toUInt64(pod_id)))) FROM flow_metrics.`application.1h` WHERE `time` >= 1694069050 AND `time` <= 1694990640 AND (pod_ns_id!=0) GROUP BY dictGet('flow_tag.pod_ns_map', 'name', (toUInt64(pod_ns_id))) AS `pod_ns` LIMIT 10"},
	}, {
		name:   "any_2",
		db:     "flow_metrics",
		...
		output: []string{"SELECT dictGet('flow_tag.region_map', 'name', (toUInt64(region_id_0))) AS `region_0`, sum(byte_tx)/(1/1) AS `Avg(byte_tx)` FROM flow_metrics.`network_map` GROUP BY dictGet('flow_tag.region_map', 'name', (toUInt64(region_id_0))) AS `region_0` LIMIT 1"},
		db:     "flow_metrics",
	}, {
		name:   "layered_0_aavg",
		input:  "select AAvg(`byte_tx`) AS `AAvg(byte_tx)`, region_0 from vtap_flow_edge_port group by region_0 limit 1",
		output: []string{"SELECT region_0, AVG(`_sum_byte_tx`) AS `AAvg(byte_tx)` FROM (SELECT dictGet('flow_tag.region_map', 'name', (toUInt64(region_id_0))) AS `region_0`, SUM(byte_tx) AS `_sum_byte_tx` FROM flow_metrics.`network_map` GROUP BY dictGet('flow_tag.region_map', 'name', (toUInt64(region_id_0))) AS `region_0`) GROUP BY `region_0` LIMIT 1"},
		db:     "flow_metrics",
	}, {
		...
		output: []string{"SELECT dictGet('flow_tag.pod_map', 'name', (toUInt64(pod_id))) AS `pod`, dictGet('flow_tag.l3_epc_map', 'name', (toUInt64(epc_id_1))) AS `l2_vpc_1` FROM flow_log.`l4_flow_log` PREWHERE (epc_id_0!=0) AND (toUInt64(epc_id) GLOBAL IN (SELECT id FROM flow_tag.l3_epc_map WHERE name != 1)) AND (epc_id_1!=0) GROUP BY dictGet('flow_tag.l3_epc_map', 'name', (toUInt64(epc_id_1))) AS `l2_vpc_1` LIMIT 1"},
	}, {
		...
		output:     []string{"WITH toStartOfInterval(time, toIntervalSecond(60)) + toIntervalSecond(arrayJoin([0]) * 60) AS `_toi` SELECT dictGet('flow_tag.pod_map', 'name', (toUInt64(pod_id))) AS `pod`, toUnixTimestamp(`_toi`) AS `toi`, divide(sum(byte)/(60/60), 60) AS `流量速率` FROM flow_metrics.`network.1m` PREWHERE (pod) GLOBAL IN (SELECT dictGet('flow_tag.pod_map', 'name', (toUInt64(pod_id))) AS `pod` FROM flow_metrics.`network.1m` PREWHERE `time` >= 1705040184 AND `time` <= 1705045184 AND (pod_id!=0) GROUP BY dictGet('flow_tag.pod_map', 'name', (toUInt64(pod_id))) AS `pod` LIMIT 5) AND `time` >= 1705040184 AND `time` <= 1705045184 AND (pod_id!=0) GROUP BY `toi`, dictGet('flow_tag.pod_map', 'name', (toUInt64(pod_id))) AS `pod` ORDER BY `toi` desc LIMIT 10000"},
	}, {
		...
		output:     []string{"WITH dictGet('flow_tag.pod_node_map', 'icon_id', (toUInt64(pod_node_id_0))) AS `client_icon_id`, dictGet('flow_tag.pod_node_map', 'icon_id', (toUInt64(pod_node_id_1))) AS `server_icon_id` SELECT pod_node_id_1 AS `pod_node_hostname_id_1`, pod_node_id_1 AS `pod_node_ip_id_1`, dictGet('flow_tag.device_map', 'hostname', (toUInt64(14),toUInt64(pod_node_id_1))) AS `pod_node_hostname_1`, dictGet('flow_tag.device_map', 'ip', (toUInt64(14),toUInt64(pod_node_id_1))) AS `pod_node_ip_1`, 'pod_node' AS `client_node_type`, 'pod_node' AS `server_node_type`, `client_icon_id`, `server_icon_id` FROM flow_metrics.`network_map.1m` PREWHERE `time` >= 1705040184 AND `time` <= 1705045184 AND (not(toUInt64(pod_node_id_1) GLOBAL IN (SELECT deviceid FROM flow_tag.device_map WHERE hostname = 'a' AND devicetype=14))) AND (not(pod_node_id_1 = 1)) AND (not(pod_node_id_1 = 2)) AND (pod_node_id_1!=0) AND (pod_node_id_1!=0) AND (pod_node_id_1!=0) AND (pod_node_id_1!=0) GROUP BY pod_node_id_1 AS `pod_node_hostname_id_1`, pod_node_id_1 AS `pod_node_ip_id_1`, dictGet('flow_tag.device_map', 'hostname', (toUInt64(14),toUInt64(pod_node_id_1))) AS `pod_node_hostname_1`, dictGet('flow_tag.device_map', 'ip', (toUInt64(14),toUInt64(pod_node_id_1))) AS `pod_node_ip_1`, `client_node_type`, `client_icon_id`, `server_node_type`, `server_icon_id` LIMIT 5"},
	}, {
		...
		output:     []string{"SELECT dictGet('flow_tag.region_map', 'name', (toUInt64(region_id))) AS `region` FROM flow_metrics.`network.1m` PREWHERE `time` >= 1705040184 AND `time` <= 1705045184 AND (host_id!=0) GROUP BY dictGet('flow_tag.region_map', 'name', (toUInt64(region_id))) AS `region` LIMIT 5"},
	}, {
		...
		output:     []string{"SELECT dictGet('flow_tag.region_map', 'name', (toUInt64(region_id_0))) AS `region_0` FROM flow_metrics.`network_map.1m` PREWHERE `time` >= 1705040184 AND `time` <= 1705045184 AND (l3_device_id_0!=0 AND l3_device_type_0=1) GROUP BY dictGet('flow_tag.region_map', 'name', (toUInt64(region_id_0))) AS `region_0` LIMIT 5"},
	}, {
		name:   "test_show_1",
		db:     "flow_metrics",
		...
		output: []string{"SELECT dictGet('flow_tag.user_map', 'name', (toUInt64(user_id))) AS `user`, user_id FROM application_log.`log` PREWHERE NOT (hasToken(body,'log')) LIMIT 1"},
	}, {
		name:   "test_showsql",
		db:     "flow_log",
